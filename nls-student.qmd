...
surplus <- Fish_data |> 
  ...
  group_by(stockid) |> 
  # creates a new column for stock id? don't see results (behind the scenes R marks individual variables within stockid (ex: APOLLPJPN))
...
fox_graph <- ggplot(data=fox_sim,aes(x=year,y=surplus, color="Data"))+
  geom_point(size=3)+
...
Metrics::rmse(actual = one_stock_predict$surplus, predicted = one_stock_predict$predict)
```
...
* nls at some point (define guesses)
* purrr::map(.x, .f = nls_function(.x))
...
all_nls_fcn<-function(surplus_df){
  ...
  nls(surplus~fox(m,carry,TBbest),
    data = surplus_df,
    start = list(m=m_guess, carry = carry_guess))
}
```
